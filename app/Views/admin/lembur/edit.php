<?php
// File: app/Views/admin/lembur/edit.php
// Halaman edit data lembur untuk admin
?>
...
<div class="card shadow mb-4">
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold">Edit Data Lembur</h6>
    ...
    <div class="card-body">
        ...
        <form id="formLembur" action="<?= site_url('admin/lembur/update/' . $lembur['idlembur']) ?>" method="post">
            <?= csrf_field() ?>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <div class="input-group">
                        <input type="hidden" id="pegawai_id" name="pegawai_id" value="<?= $lembur['pegawai_id'] ?>" required>
                        <input type="text" id="pegawai_nama" name="pegawai_nama" class="form-control <?= (session()->has('errors') && isset(session('errors')['pegawai_id'])) ? 'is-invalid' : '' ?>" placeholder="Pilih pegawai" readonly value="<?= $lembur['namapegawai'] ?> (<?= $lembur['nik'] ?>)" required>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" id="tanggallembur" name="tanggallembur" value="<?= old('tanggallembur') ?? $lembur['tanggallembur'] ?>" class="form-control <?= (session()->has('errors') && isset(session('errors')['tanggallembur'])) ? 'is-invalid' : '' ?>" required>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="time" id="jammulai" name="jammulai" value="<?= old('jammulai') ?? date('H:i', strtotime($lembur['jammulai'])) ?>" class="form-control <?= (session()->has('errors') && isset(session('errors')['jammulai'])) ? 'is-invalid' : '' ?>" required>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="time" id="jamselesai" name="jamselesai" value="<?= old('jamselesai') ?? date('H:i', strtotime($lembur['jamselesai'])) ?>" class="form-control <?= (session()->has('errors') && isset(session('errors')['jamselesai'])) ? 'is-invalid' : '' ?>" required>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <textarea id="alasan" name="alasan" rows="4" class="form-control <?= (session()->has('errors') && isset(session('errors')['alasan'])) ? 'is-invalid' : '' ?>" required><?= old('alasan') ?? $lembur['alasan'] ?></textarea>
                    <div class="invalid-feedback error-alasan"></div>
                </div>
            </div>

            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                <a href="<?= site_url('admin/lembur') ?>" class="btn btn-secondary">
        ...
        $('#formLembur').on('submit', function(e) {
            ...
            $.ajax({
                ...
                complete: function() {
                    ...
                    $('#btnSimpan').html('<i class="bi bi-save me-1"></i> Simpan Perubahan');
                }
            });
        });
    });
